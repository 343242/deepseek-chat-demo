...
|------|-------|
| `DIRECT_ANSWER` | 无 |
| `RETRIEVAL` | hybridSearch, rerank |
| `DEEP_RETRIEVAL` | vectorSearch, hybridSearch, rerank, queryRewrite, parentDocLookup |
...
> **⚠️ 可行性验证**：Spring AI 1.1.6 的 `ToolCallAdvisor` 在 ReAct 循环中，每轮中间 `ChatResponse` 是否暴露 `usage` 元数据需 PoC 验证。若不暴露，改为基于输入文本估算（字符数 / 4）。
