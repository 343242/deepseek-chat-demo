...
│  💬 聊天            │  模型配置                       [↻ 刷新模型列表]  │
│  📚 知识库          │  ┌─ ⚠️ 提示条 ──────────────────────────────────┐ │
│  👥 团队            │  │ 参数对全部用户生效；当前版本仅存储，对话暂未  │ │
│  📊 用量统计        │  │ 按此参数调用（后端接线中）                    │ │
│                     │  └─────────────────────────────────────────────┘ │
│  ⚙️ 后台管理(ADMIN) │  筛选: [全部能力 ▾]  [仅已自定义 ▾]   🔍 搜索模型 │
│                     │  ┌─ 模型表 ────────────────────────────────────┐ │
│                     │  │ DeepSeek                                    │ │
│                     │  │  deepseek-v4-flash  对话      ●可用  [已自定义] ⋮│
│                     │  │  deepseek-reasoner  对话      ●可用  [默认]   ⋮│
│                     │  │ 智谱 AI                                     │ │
│                     │  │  embedding-3        向量化     ●可用  [默认]   ⋮│
│                     │  │  qwen3-max          对话      ○不可用 [默认]   ⋮│
│                     │  └────────────────────────────────────────────┘ │
└─────────────────────┴──────────────────────────────────────────────────┘
```
...
│ [ 4096            ]   1 – 128000         │
│                                          │
│ Frequency Penalty（重复惩罚）    0        │
│ ├●──────────────────┤  -2 – 2，步进 0.1  │
│                                          │
│ Presence Penalty（话题惩罚）     0        │
│ ├●──────────────────┤  -2 – 2，步进 0.1  │
│                                          │
│ ⚠️ 保存后对全部用户的该模型调用生效         │
│                     [取消]  [保存参数]    │
