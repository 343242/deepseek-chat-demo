 ...
 *   <li>IO 密集型：N_cpu * 2（等待时间占比高时乘数可继续增大）</li>
 * </ul>
...
public final class ThreadPoolConstants {
    ...
    /** CPU 密集型最大线程数 = CPU_COUNT * 2 */
    public static int cpuMax() { return CPU_COUNT * 2; }

    // ---- IO-bound (network/disk wait: MinIO, Embedding API, PGvector, SSE) ----

    /** IO 密集型核心线程数 = CPU_COUNT * 2 */
    public static int ioCore() { return CPU_COUNT * 2; }

    /** IO 密集型最大线程数 = CPU_COUNT * 4 */
    public static int ioMax() { return CPU_COUNT * 4; }
    ...
    /** 轻量级最大线程数 = CPU_COUNT * 2 */
    public static int lightMax() { return CPU_COUNT * 2; }
}
