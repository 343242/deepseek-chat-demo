...
public class McpClientTransportConfiguration {
    ...
    public List<McpSyncClient> mcpSyncClients(
        ...
        McpClientTransportProperties.Client clientCfg = props.getClient();
        if (clientCfg == null || !"SYNC".equalsIgnoreCase(clientCfg.getType())) {
            log.warn("MCP: client.type 非 SYNC 或未配置，跳过 transport 构建");
            return List.of();
        }

        McpClientTransportProperties.StreamableHttp streamableHttp = clientCfg.getStreamableHttp();
        ...
        Duration requestTimeout = parseDuration(clientCfg.getRequestTimeout(), Duration.ofSeconds(30));
