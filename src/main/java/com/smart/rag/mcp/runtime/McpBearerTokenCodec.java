...
/** Encodes the MCP bearer token as a versioned AES-GCM cipher/IV envelope. */
@Component
public class McpBearerTokenCodec {
    ...
    public String encode(String bearerToken) {
        ...
        return VERSION + ":" + encoder.encodeToString(encrypted.cipher())
    ...
    public String decode(@Nullable String stored) {
        ...
        try {
            ...
            byte[] cipher = decoder.decode(parts[1]);
            byte[] iv = decoder.decode(parts[2]);
            if (iv.length != GCM_IV_BYTES) {
                throw unreadable(null);
            }
            return secretCipher.decrypt(cipher, iv);
