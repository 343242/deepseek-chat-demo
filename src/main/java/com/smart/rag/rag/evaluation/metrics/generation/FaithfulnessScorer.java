package com.smart.rag.rag.evaluation.metrics.generation;

import com.smart.rag.common.util.JsonExtractor;
import com.smart.rag.rag.evaluation.judge.LlmJudge;
...
public class FaithfulnessScorer {
     ...
     * @return 忠实度分数（0-1），Judge 失败返回 -1
     */
    public double score(String answer, String context) {
        // Step 1: 提取 claims
        List<String> claims = extractClaims(answer);
        if (claims.isEmpty()) {
            log.warn("No claims extracted from answer");
            return 1.0; // 无声明 = 无幻觉 = 满分
        }
    ...
    private double verifyClaims(List<String> claims, String context) {
        ...
        try {
            ...
            if (result.containsKey("faithfulness_score")) {
                return ((Number) result.get("faithfulness_score")).doubleValue();
            }
