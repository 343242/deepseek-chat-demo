 ...
 * <p>
 * 核心逻辑抽取为包级 {@link #rerankOnly(Query, List)}，供 {@link RerankThenMmrPostProcessor}
 * 异步调用（B3 无状态：纯函数 + 只读 reranker/topN 字段，跨请求共享安全）。
 */
public class RerankDocumentPostProcessor implements DocumentPostProcessor {
    ...
    public List<Document> process(Query query, List<Document> documents) {
        return rerankOnly(query, documents);
    }

    /**
     * 执行 Rerank 精排（写 {@code rerankScore} metadata），保留降级契约。
     * <p>
     * 降级契约（R1-M8，不中断检索链路）：
     * <ul>
     *   <li>空文档 → 空</li>
     *   <li>空/blank 查询 → 原样透传（无语义无法精排）</li>
     *   <li>rerank API 失败/超时/熔断 → 原样透传（不抛异常——否则传播到 chat call 触发跨模型 fallback 误判）</li>
     *   <li>rerank 返回空 → 原样透传 + warn</li>
     * </ul>
     *
     * @param query     查询（blank 时透传）
     * @param documents 待精排文档
     * @return 精排后文档（带 rerankScore），或降级透传的原文档
     */
    List<Document> rerankOnly(Query query, List<Document> documents) {
