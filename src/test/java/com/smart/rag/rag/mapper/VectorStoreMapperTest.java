...
class VectorStoreMapperTest {
    ...
    class Bm25Search {
        ...
        void bm25Search_emptyResult() {
            ...
            List<org.springframework.ai.document.Document> result = mapper.bm25Search("jiebacfg", "query", "userId", "1", 5);
            assertThat(result).isEmpty();
        }

        @Test
        @DisplayName("topK=1 参数传递正确")
        @SuppressWarnings("unchecked")
    ...
    class BatchFetchParents {
        ...
        void batchFetchParents_emptyInput() {
            Map<String, org.springframework.ai.document.Document> result = mapper.batchFetchParents(Collections.emptySet());
            assertThat(result).isEmpty();
